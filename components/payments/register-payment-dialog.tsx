        ...
        <div>
          ...
          {hasDiff && (
            <p className={`mt-1 flex items-center gap-1 text-[11px] font-medium ${difference > 0 ? "text-emerald-600" : "text-amber-600"}`}>
              ...
              {difference > 0 ? "+" : ""}€{difference.toFixed(2)} vs atteso
            </p>
          )}
        </div>
      </div>
          ...
          {sessions.map((s) => {
            ...
            return (
              <div key={s.id} className="flex items-center justify-between text-xs">
                <span className="truncate text-foreground">{meta.exam_name ?? "Sessione"}</span>
    ...
    return (
      <Sheet open onOpenChange={(o) => !o && onClose()}>
        <SheetContent
          ...
          style={{ height: "92dvh" }}
        >
          <SheetTitle className="sr-only">Registra Pagamento</SheetTitle>
